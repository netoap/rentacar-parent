...
import com.example.auth.ports.out.UpdateUserPort;
...
public class UserController {
    ...
    public ResponseEntity<UserProfileResponse> getCurrentUser(Authentication authentication) {
        ...
        return loadUserPort.findByUsername(username)
                .map(user -> ResponseEntity.ok(
                        new UserProfileResponse(user.getId(), user.getUsername(), user.getRoles(), user.getCreatedAt(), user.isActive()))
                )
                .orElse(ResponseEntity.status(404).build());
    }

    //    GET /users?username=admin&role=ROLE_ADMIN&page=0&size=5&sort=createdAt,desc
    @Operation(
    ...
    public ResponseEntity<PagedResponse<UserProfileResponse>> getAllUsers(
        ...
        List<UserProfileResponse> userProfiles = userPage.getContent().stream()
                .map(user -> new UserProfileResponse(user.getId(), user.getUsername(), user.getRoles(), user.getCreatedAt(), user.isActive()))
                .toList();
    ...
    public ResponseEntity<UserProfileResponse> changePassword(
        ...
        return ResponseEntity.ok(new UserProfileResponse(user.getId(), user.getUsername(), user.getRoles(), user.getCreatedAt(), user.isActive()));
    }
    ...
    public ResponseEntity<UserProfileResponse> updateRoles(
        ...
        return ResponseEntity.ok(new UserProfileResponse(updated.getId(), updated.getUsername(), updated.getRoles(), updated.getCreatedAt(), updated.isActive()));
    }
